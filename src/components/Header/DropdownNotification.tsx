...
const DropdownNotification = () => {
  ...
  const {} = useQuery({
    queryFn: () => Axios.get(`${NOTIFICATIONS}?limit=10&page=1`),
  ...
  const showNotification = notification?.map(
    (notification: notification, key) => (
      <Link
        to={
          notification.scope === 'class'
            ? '/classes/my-classes'
            : notification.scope === 'product'
            ? '/lms/products'
            : notification.scope === 'course'
            ? '/lms/courses'
            : notification.scope === 'comment' || notification.scope === 'post'
            ? '/comments/post/' + notification.relatedId
            : ''
        }
        key={key}
      >
        <div className="flex flex-col gap-2.5 border-t border-stroke px-4.5 py-3 hover:bg-gray-2 dark:border-strokedark dark:hover:bg-meta-4">
          ...
          <p className="text-xs">
            <TimeAgo
              minPeriod={60}
              date={adjustDateByHours(notification?.createdAt, -3)}
            />
